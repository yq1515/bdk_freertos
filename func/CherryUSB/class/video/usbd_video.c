...
uint32_t usbd_video_h264_payload_fill(uint8_t *input, uint32_t input_len, uint8_t *output, uint32_t *out_len)
{
    ...
    uint32_t last_packet_size;
    uint32_t picture_pos = 0;
    uint32_t max_payload_transfer_size = g_usbd_video.probe.dwMaxPayloadTransferSize;
...
{
    const uint32_t max_payload_transfer_size = g_usbd_video.probe.dwMaxPayloadTransferSize;
...
{
    return g_usbd_video.probe.dwMaxPayloadTransferSize;
}
