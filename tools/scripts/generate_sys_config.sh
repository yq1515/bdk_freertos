...
if [ "$2" == "" ]; then
	old_sys_config=include/sys_config.h
fi
...
case $1 in
	bk7231u)
		new_sys_config=include/sys_config_bk7231u.h
		;;
	bk7231n)
		new_sys_config=include/sys_config_bk7231n.h
		;;
	bk7251)
		new_sys_config=include/sys_config_bk7251.h
		;;
	bk7231)
		new_sys_config=include/sys_config_bk7231.h
		;;
	*)
		new_sys_config=include/$1.h
esac
...
rm -f .config
echo '#include "include/sys_config.h"' > config.c
sed -n '/^#define/p' include/sys_config.h | awk '{print $2}' | sort -d | uniq | awk '{print "valueOf_"$1"="$1}' >> config.c
