...
/*__attribute__((section(".noncacheable"))) */struct usbd_core_cfg_priv {
    /** Setup packet */
...
void usbd_event_ep_in_complete_handler(uint8_t ep, uint32_t nbytes)
{
    if (ep == USB_CONTROL_IN_EP0) {
        ...
        if (usbd_core_cfg.ep0_data_buf_residue)
